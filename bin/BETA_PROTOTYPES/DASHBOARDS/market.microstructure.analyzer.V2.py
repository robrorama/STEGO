#!/usr/bin/env python3
# SCRIPTNAME: ok.market.microstructure.analyzer.V2.py
...
def main():
    ...
    for ticker in args.tickers:
        ...
        if vol_stats:
            ...
            fig_vol.update_layout(title=f'{ticker} Realized Volatility Term Structure', template='plotly_dark')
        ...
        if ticker in gex_data and gex_data[ticker]:
            ...
            fig_gex.update_layout(title=f'{ticker} Dealer Gamma Exposure Profile', xaxis_title='Strike', yaxis_title='GEX ($)', template='plotly_dark')
            ...
            fig_press.update_layout(title=f'{ticker} Dealer Greek Pressure (Vanna/Charm)', template='plotly_dark')
            ...
            if len(raw_g) > 10:
                ...
                fig_3d = go.Figure(data=[go.Scatter3d(
                    x=strikes, y=expiries, z=raw_g['vanna'], 
                    mode='markers', 
                    marker=dict(size=4, color=raw_g['vanna'], colorscale='Plasma', showscale=True, opacity=0.9)
                )])
                fig_3d.update_layout(
                    title=f'{ticker} Vanna Surface (Strike vs DTE vs Vanna)',
                    template='plotly_dark',
                    scene=dict(
                        xaxis_title='Strike',
                        yaxis_title='DTE',
        ...
        if ticker in intraday_data:
            ...
            if not idf.empty:
                ...
                fig_intra.update_layout(title=f'{ticker} Intraday Microstructure', template='plotly_dark')
        ...
        if liq_stats:
            ...
            fig_liq.update_layout(title=f'{ticker} Liquidity Stress (Amihud)', template='plotly_dark')
        ...
        if macro_emb:
            ...
            fig_rad.update_layout(title=f'{ticker} Macro Factor Correlations', template='plotly_dark')
        ...
        if regime_info:
            ...
            # Loop through regimes to create legend entries
            unique_regimes = sorted(data['regime'].unique())
            regime_colors = {0: '#00CC96', 1: '#EF553B', 2: '#636EFA', 3: '#AB63FA'}
            ...
            for r in unique_regimes:
                ...
                fig_reg.add_trace(go.Scatter(
                    ...
                    mode='markers',
                    name=f'Regime {r}',
                    marker=dict(size=6, color=regime_colors.get(r, '#ffffff'))
                ))

            fig_reg.update_layout(title=f'{ticker} Regime Classification (HMM/Clustering)', template='plotly_dark')
        ...
        if index_ticker in daily_data:
            ...
            if comps:
                ...
                if disp_res:
                    ...
                    fig_disp.update_layout(title='Index vs Component Dispersion Index', template='plotly_dark')
                    ...
                    fig_heat.update_layout(title='Component Correlation Matrix', template='plotly_dark')
